...
export function EditHotelModal({
  ...
  const onSubmit = async (data: HotelFormData) => {
    ...
    try {
      ...
      // Read and update mockData.json
      const response = await fetchWithAuth(`/api/hotels/${hotel._id}`, {
        method: "PATCH",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Cập nhật thông tin khách sạn (chỉ có thể chỉnh sửa khi trạng thái là DRAFT)
          </DialogDescription>
        </DialogHeader>
